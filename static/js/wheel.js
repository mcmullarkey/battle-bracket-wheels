...
(function () {
  ...
  // Flip the battle pointer direction to match the winner at reveal time.
  //
  // The .battle-pointer is a directional right-pointing triangle (SVG polygon).
  // Left-winner  → add .pointer-left class → CSS scaleX(-1) → points left.
  // Right-winner → remove .pointer-left     → identity        → points right.
  //
  // Uses querySelectorAll(".battle-pointer") to catch ALL instances, including
  // stale duplicates from repeated battles (§3.3). Called from revealResults
  // (at REVEAL_DELAY_MS) so the OOB swap is guaranteed complete — NOT
  // synchronously in the spin-wheel handler (which fires before the swap).
  //
  // Solo spins (items.length <= 1) are skipped — no winner, no flip (AC-5).
  function revealPointerWithResults() {
    var items = window.__lastSpinItems;
    if (!items || items.length <= 1) return;

    // items[0] = left wheel, items[1] = right wheel (per handler ordering).
    var leftWins = items[0] && items[0].winner === true;

    var allPointers = document.querySelectorAll(".battle-pointer");
    for (var i = 0; i < allPointers.length; i++) {
      if (leftWins) {
        allPointers[i].classList.add("pointer-left");
      } else {
        allPointers[i].classList.remove("pointer-left");
      }
    }
  }
  ...
  function revealResults() {
    ...
    // Flip the battle pointer direction to match the winner.
